  ...
  (let [catalogs [{"certname" "foo" "edges" [{"a" "b"}] "resources" [{"tags" ["a"]}]}
    ...
    (testing "nothing"
      (let [catalogs nil
            ...
            facts nil
            preserved-host-maps []
            host-maps (benchmark/recover-and-generate-host-maps 3 0 "pdb.test" false
                                                                catalogs reports facts
                                                                "_" preserved-host-maps)]
             ...
             [{:host "host-0"
              ...
              {:host "host-2"
               :catalog {"certname" "host-2"
                         "edges" [{"g" "h"}]
                         "resources" [{"tags" ["preserved2" "pdb.test"]}]}}]
            host-maps (benchmark/recover-and-generate-host-maps 3 0 "pdb.test" true
                                                                catalogs nil facts
                                                                "_" preserved-host-maps)]
        (is (= [{:host "host-0"
                 :catalog {"certname" "host-0"
